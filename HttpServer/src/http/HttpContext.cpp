 ...
 * 解析HTTP请求报文
 * 
 * 该函数从缓冲区中逐步解析HTTP请求，包括请求行、请求头和请求体。
 * 采用状态机方式处理，支持分片接收和数据不完整时的等待。
 * 
 * @param buf 输入缓冲区，包含待解析的原始HTTP数据
 * @param receiveTime 请求接收的时间戳，用于设置请求对象的接收时间
 * @return bool 解析是否成功。返回false表示遇到语法错误；返回true表示解析成功或需要等待更多数据
 */
...
/**
 * @brief 解析 HTTP 请求行（Request Line）
 * 
 * 从给定的字符范围中解析 HTTP 请求的第一行，格式通常为：
 * METHOD SP URI SP HTTP-Version CRLF
 * 
 * 该函数会提取并设置请求的方法（Method）、路径（Path）、查询参数（Query Parameters）
 * 以及 HTTP 版本（Version）到内部 request_ 对象中。
 * 
 * @param begin 指向请求行起始位置的指针
 * @param end   指向请求行结束位置的指针（不包含末尾的 CRLF，通常指向空格或结尾）
 * @return bool 如果请求行格式合法且解析成功返回 true，否则返回 false
 */
